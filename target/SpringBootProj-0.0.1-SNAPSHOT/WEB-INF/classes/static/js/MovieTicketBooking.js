...
function searchWholeCards() {
  ...
  for (i = 0; i < cards.length; i++) {
    title = cards[i].querySelector(".imgcontent");
...
function searchMovie() {
  ...
  filter = input.value.toUpperCase();
  cardContainer = document.getElementsByClassName("cards");
  cards = document.getElementsByClassName("card");
  for (i = 0; i < cards.length; i++) {
...
function searchTheatre() {
  ...
  cards = document.getElementsByClassName("card");
  for (i = 0; i < cards.length; i++) {
    title = cards[i].querySelector(".card-theatre");
    if (title.innerText.toUpperCase().indexOf(filter) > -1) {
      cards[i].style.display = "";
    } else {
      cards[i].style.display = "none";
...
function processMovieDates(map){
	...
	for(data in map){
		...
		const curdate = new Date(currdate);
		const diffTime = inputdate.getTime() - curdate.getTime();
		const diffDays = diffTime / (1000 * 60 * 60 * 24);
		if(diffDays >= 0){
			$("#date_of_reservation").attr('min', data);
			$("#date_of_reservation").attr('max', map[data]);
		}else{
			$("#date_of_reservation").attr('min', currdate);
			$("#date_of_reservation").attr('max', map[data]);
		}
	}
}
